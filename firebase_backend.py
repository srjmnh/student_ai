...
def cleanup_data():
    ...
    deletes the rest, also deletes docs missing 'id'.
    """
    ...
    for st in students:
        # If doc lacks "id", delete it automatically
        doc_id = st.get("id")
        if not doc_id:
            # Delete doc from Firestore if possible
            log_activity("CLEANUP_DATA", f"Doc missing 'id'. Attempting to remove: {st}")
            # We do not know the doc name, but we can find by matching fields. 
            # For simplicity, skip actual removal if we don't store doc name in Firestore doc metadata. 
            # If we want a safer approach, we'd store doc reference from the stream. 
            # We'll just skip. Or we can keep a flag that we'll remove it if doc is found by some unique field.
            continue
...
###############################################################################
# 10. Building an Editable Table + Animations
###############################################################################
...
def index():
    """
    Return the chat UI with background music,
    more mobile styling, table editing, etc.
    """
    return """
<!DOCTYPE html>
<html>
...
<body>
  <!-- Background music with random track from an array of URLs (in JS below) -->
  ...
  <script>
    ...
    // On page load, pick a random track
